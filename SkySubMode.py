...
def calc_stats(obj_file, obj_data, sky_model, logfile):

    # Object stats
    # No clipping
    objarr = obj_data[np.isfinite(obj_data)]
    median_noclip = np.median(objarr)
    mean_noclip = np.mean(objarr)
    mode_noclip = 3 * median_noclip - 2 * mean_noclip
    # 3-sigma clipping
    clipped = sigma_clip(objarr, sigma=3, maxiters=3)
    objarr_clip = clipped.compressed()
    median_clip = np.median(objarr_clip)
    mean_clip = np.mean(objarr_clip)
    mode_clip = 3 * median_clip - 2 * mean_clip

    with open(logfile, "a") as f:
        f.write("\n")
        f.write(f"OBJECT: {obj_file}\n")
        f.write(f"Mean: {mean_noclip}; Median: {median_noclip}; Mode: {mode_noclip}\n")
        f.write(f"3-sigma clipped\n")
        f.write(f"Mean: {mean_clip}; Median: {median_clip}; Mode: {mode_clip}\n")

    # Sky model stats
    # No clipping
    skyarr = sky_model[np.isfinite(sky_model)]
    median_noclip = np.median(skyarr)
    mean_noclip = np.mean(skyarr)
    mode_noclip = 3 * median_noclip - 2 * mean_noclip
    # 3-sigma clipping
    clipped = sigma_clip(skyarr, sigma=3, maxiters=3)
    skyarr_clip = clipped.compressed()
    median_clip = np.median(skyarr_clip)
    mean_clip = np.mean(skyarr_clip)
    mode_clip = 3 * median_clip - 2 * mean_clip
...
while j < num_detectors:
    ...
    for obj_file, sky_files in results.items():
        ...
        if len(sky_files) > 2:
            ...
            with open(logfile, 'a') as f:
                sys.stdout = f
                print(f"Skipping {obj_file}: Only {len(sky_files)} sky file(s)")
                sys.stdout = sys.__stdout__
