...
def divide_sky(obj_file, sky_files, ext_dir, outdir):
    ...
    good = sky_model[np.isfinite(sky_model)]
    median = np.median(good)
    mean = np.mean(good)
    mode = 3*median - 2*mean
    sky_norm = sky_model / mode

    result = obj_data / sky_norm

    time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    sky_key = f"{time}; Sky Flat-fielded"
    hdr["SKYFLAT"] = sky_key
    hdr["PYSFLAT"] = ('True', "SkyFlat.py Flag")
    ...
    outfile = os.path.join(
        outdir,
        obj_file.replace("fdtc4n", "ffdtc4n")
    )
...
while j < num_detectors:
    ...
    for obj_file, sky_files in results.items():
        ...
        divide_sky(obj_file, sky_files, ext_dir, outdir)
    j = j + 1
